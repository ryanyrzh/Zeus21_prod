...
import numpy as np
# === BEGIN EDIT (par/perp eta split): added imports needed by runclass below ===
...
# === BEGIN EDIT (cosmology_v2: par/perp eta split) ===
# Use the patched Cosmo_Parameters from inputs_v2, which honors
# USE_ETA_PARPERP_SPLIT in both runclass() and run_correlations().
from .inputs_v2 import Cosmo_Parameters
# === END EDIT (cosmology_v2: par/perp eta split) ===

# === BEGIN EDIT (cosmology_v2: par/perp eta split) ===
# Rewritten cosmo_wrapper. The previous version imported a non-existent
# `Correlations` class from zeus21.correlations[_v2] and instantiated
# Cosmo_Parameters with the wrong arg count, so it could never run.
#
# This version delegates all CLASS + correlation-function setup to
# inputs_v2.Cosmo_Parameters.__post_init__, which now:
#   * dispatches in runclass() between legacy `k_eta`/`P_eta` and the
#     split `k_etapar`/`k_etaperp` keys based on USE_ETA_PARPERP_SPLIT;
#   * dispatches in run_correlations() between building xiEta_RR_CF and
#     building (xiEtaPar_RR_CF, xiEtaPerp_RR_CF).
#
# `Cosmo_Parameters_Input` is treated as a duck-typed object: any of its
# attributes that match a Cosmo_Parameters field are forwarded as kwargs.
# Callers can therefore pass a SimpleNamespace, a dataclass, or a custom
# class.
#
# Returns (CosmoParams, ClassCosmo, HMFintclass).
_COSMO_PARAM_FIELDS = (
    'omegab', 'omegac', 'h_fid', 'As', 'ns', 'tau_fid',
    'kmax_CLASS', 'zmax_CLASS', 'zmin_CLASS',
    'Rs_min', 'Rs_max',
    'Flag_emulate_21cmfast', 'USE_RELATIVE_VELOCITIES',
    'USE_ETA_PARPERP_SPLIT', 'HMF_CHOICE',
)

def cosmo_wrapper(User_Parameters, Cosmo_Parameters_Input):
    """
    Wrapper that instantiates the full cosmology pipeline for the v2
    code path. Returns (Cosmo_Parameters, ClassCosmo, HMF_interpolator).

    Parameters
    ----------
    User_Parameters : zeus21.User_Parameters
    Cosmo_Parameters_Input : object
        Any object whose attributes are a subset of the Cosmo_Parameters
        fields (omegab, omegac, ..., USE_RELATIVE_VELOCITIES,
        USE_ETA_PARPERP_SPLIT, HMF_CHOICE). A types.SimpleNamespace or
        a small dataclass works fine.

    The boolean ``Cosmo_Parameters_Input.USE_ETA_PARPERP_SPLIT`` selects:
      * False (default): legacy isotropic eta correlation function.
      * True: parallel/perpendicular split (xiEtaPar_RR_CF and
        xiEtaPerp_RR_CF are populated; xiEta_RR_CF is NOT).
    """
    kwargs = {
        name: getattr(Cosmo_Parameters_Input, name)
        for name in _COSMO_PARAM_FIELDS
...
# === END EDIT (cosmology_v2: par/perp eta split): cosmo_wrapper ===


# === BEGIN EDIT (par/perp eta split): new runclass() supporting two
# mutually exclusive modes for the v_cb^2 ("eta") correlation function,
# selected by CosmologyIn.USE_ETA_PARPERP_SPLIT (default False = legacy).
# Only the keys for the selected mode are written to ClassCosmo.pars,
# so a stale consumer of the other mode fails loudly. ===
def runclass(CosmologyIn):
    """
    Set up CLASS cosmology. Takes CosmologyIn class input and returns CLASS Cosmology object.

    Two distinct treatments of the v_cb^2 ("eta") correlation function are
    supported, selected by the boolean attribute
    ``CosmologyIn.USE_ETA_PARPERP_SPLIT`` (default False if absent):

      * False (legacy):
            Stores a single isotropic eta power spectrum on
            ``ClassCosmo.pars`` under keys ``k_eta`` and ``P_eta``,
            built from the integrand ``6*psi0**2 + 3*psi2**2``.

      * True (parallel/perpendicular split):
            Stores two eta power spectra on ``ClassCosmo.pars`` under
            keys ``k_etapar``/``P_etapar`` and ``k_etaperp``/``P_etaperp``,
            built from the integrands ``6*(psi0 + psi2)**2`` and
            ``6*(psi0 - psi2/2)**2`` respectively. These feed the
            modified four-term factor in
            ``Power_Spectra.get_xi_Sum_2ExpEta``:
                ((1 - K*xiEtapar) * (1 - K*xiEtaperp)**2)**(1/2)
            in place of the legacy (1 - K*xiEta)**(3/2).

    The two modes are mutually exclusive: only the keys for the selected
    mode are set, so a stale consumer of the other mode fails loudly.
    """
    USE_ETA_PARPERP_SPLIT = getattr(CosmologyIn, 'USE_ETA_PARPERP_SPLIT', False)
    ...
    ClassCosmo.pars['USE_ETA_PARPERP_SPLIT'] = USE_ETA_PARPERP_SPLIT
    ...
    if CosmologyIn.USE_RELATIVE_VELOCITIES == True:
        ...
        velArr = np.geomspace(0.01, constants.c_kms, 1000)
        ...
        # === BEGIN EDIT (par/perp eta split): exactly one of the two
        # branches below executes. Each writes ONLY the keys that match
        # its mode so consumers cannot silently mix conventions. ===
        if USE_ETA_PARPERP_SPLIT:
            # Parallel/perpendicular split: store ONLY the par and perp keys.
            ...
            # NOTE: 'k_eta'/'P_eta' intentionally NOT set so a stale
            # isotropic consumer fails loudly rather than using wrong CF.
        else:
            # Legacy isotropic eta: store ONLY the single-key version.
        ...
        ClassCosmo.pars['sigma_vcb'] = 1.0

    return ClassCosmo
# === END EDIT (par/perp eta split): end of new runclass() ===
